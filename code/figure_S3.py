...
ds_temp = xr.open_dataset(file_fp+'ERA5_lapserates_monthly.nc', decode_times=False);
tt = ds_temp['lapserate'].values # deg C m-1 * 1000
ds_temp.close()
resolution = 0.25
...
tt = tt[55:74,:,:]
tt = np.nanmean(tt,axis=0)
temp = tt.copy()
temp[:,0:int(n/2)] = tt[:,int(n/2):n]
temp[:,int(n/2):n] = tt[:,0:int(n/2)]
temp = temp * 1000

ds_prcp = xr.open_dataset(file_fp+'ERA5_geopotential.nc', decode_times=False);
pp = ds_prcp['z'].values
ds_prcp.close()
pp = np.nanmean(pp.reshape(85,12,721,1440), axis=1)
pp = pp[55:74,:,:]
...
ax1.set_title('Mean annual geopotential height during 1995 ~ 2014 (* 10$^3$ m)', fontweight='bold',loc='center', pad=2)
...
im1  = ax1.imshow(prcp, extent=extents, transform=ccrs.PlateCarree(), alpha=0.8,
                  norm=norm, cmap='viridis')

char1 = fig.colorbar(im1, ax=ax1, ticks=np.linspace(0, 6, 7), extend='both',
...
ax2.set_title('Mean annual temperature lapse rate during 1995 ~ 2014 (* 10$^{-3}$ °C/m)', fontweight='bold',loc='center', pad=2)
...
norm = mcolors.Normalize(vmin=-7, vmax=-2)
im2  = ax2.imshow(temp, extent=extents, transform=ccrs.PlateCarree(), alpha=0.8,
                  norm=norm, cmap='RdYlBu_r')

char2 = fig.colorbar(im2, ax=ax2, ticks=np.linspace(-7, -2, 6), extend='both',
                     shrink=0.7, aspect=20, pad=0.06, orientation='vertical')
char2.ax.tick_params(direction='in', size=2, width=0.5, labelsize=7)

out_pdf = '/Users/wyan0065/Desktop/AAR-disequilibrium/AABR/manuscript/' + 'figure_S3.png'
