...
def calc_stats_AAR(data):
    ...
    if (np.isnan(data)).all():
        stats = np.zeros(2) * np.nan
    else:
        stats = np.append(stats, np.nanmedian(data, axis=0)) # 'median'
        stats = np.append(stats, median_abs_deviation(data, axis=0, nan_policy='omit')) # Compute the median absolute deviation of the data
    return stats

def compute_AAR(gdir, massbaltotal_monthly, ELA_annual, bin_massbalclim_annual, water_level):
    """
    Calculate glacier AAR and the relative variables for a given glacier

    Parameters
    ----------
    fls : OGGM GlacierDirectory
    masbaltotal_monthly : xarray dataset
        output_glac_massbaltotal_monthly
    ELA_annual : xarray dataset
        output_glac_ELA_annual
    frontalablation_monthly : xarray dataset
    ...
    nyear = int(args.gcm_endyear-args.gcm_startyear+1)
    ...
    mb_mwea = gdir.mbdata['mb_mwea']
    if gdir.is_tidewater:
        mb_clim_mwea = gdir.mbdata['mb_clim_mwea']
    else:
        mb_clim_mwea = mb_mwea;
    
    nfls = gdir.read_pickle('inversion_flowlines');
    nfl = nfls[0];
    n=len(nfl.surface_h)
    # dx, map_dx, surface_h, bed_h, rgi_id, water_level
    # area_km2, area_m2, bin_area_m2, flows_to_indice, inflow_indices, length_m, 
    # normals, point_lats, point_lons, surface_h, terminus_index, thick, 
    # volume_bsl_km3, volume_bsl_m3, volume_bwl_km3, volume_bwl_m3, widths, 
    try:
    ...
    fl = fls[0];
    bin_area = fl.bin_area_m2[0:n];
    tot_area = fl.area_m2;
    
    fa_ts = tot_area*(mb_mwea-mb_clim_mwea);
    
    ela = ELA_annual[0:-1,:];
    
    massbaltotal_monthly = massbaltotal_monthly.reshape(nyear,12,sims);
    mb_ts = massbaltotal_monthly.sum(axis=1);
    bin_massbalclim_annual = bin_massbalclim_annual[:,0:nyear,:];
    
    
    intercept_AAR = np.zeros(sims) * np.nan
    steady_AAR    = np.zeros(sims) * np.nan
    ...
    for i in range(sims):
        ...
        for j in range(nyear):
            tot_area_above = 0;
            is_above = fl.surface_h[0:n] > ela[j,i];
            tot_area_above = sum(bin_area * is_above);
            
            _aar = tot_area_above/tot_area;
            
            if _aar >= 0.05 and _aar <= 0.95 and np.isnan(_aar)+np.isnan(mb_ts[j,i])+np.isnan(ela[j,i])==0:
                smb = np.append(smb, mb_ts[j,i]+fa_ts)
        ...
        # ========================== Calculate mean AAR0 by estimating steady state ==========================
        AAR = [];
        ela_yearly = [];
        AAR_yearly = [];
        for j in range(nyear):
            ...
            tot_area_above = 0;
            from_steady = mb_ts[j,i]+fa_ts;
            bias = -from_steady/tot_area;
            bin_massbalclim = bin_massbalclim_annual[:,j,i] + bias;
            is_above = bin_massbalclim>0;
            tot_area_above = sum(bin_area * is_above);
            _aar = tot_area_above/tot_area
            if _aar_yearly >= 0.05 and _aar_yearly <= 0.95 and _aar >= 0.05 and _aar <= 0.95 and \
                np.isnan(_aar_yearly)+np.isnan(_aar)+np.isnan(ela[j,i])==0:
                AAR = np.append(AAR,_aar)
                ela_yearly = np.append(ela_yearly, ela[j,i])
                AAR_yearly = np.append(AAR_yearly, _aar_yearly)

        if len(AAR) >=5:
            if steady_AAR[i] >= 0.05 and steady_AAR[i] <= 0.95:
                steady_AAR[i] = AAR.mean()
            else:
                steady_AAR[i] = np.nan;
        else:
            steady_AAR[i] = np.nan;
    ...
    results = fa_ts;
    return results, intercept_AAR, steady_AAR
...
def create_xrdataset(glacier_rgi_table, dates_table, option_wateryear=pygem_prms.gcm_wateryear, 
    ...
    if export_extra_vars:
        ...
        output_coords_dict['glac_prec'] = collections.OrderedDict([('glac', glac_values), 
                                                                   ('dim', dim)])
        output_coords_dict['glac_temp'] = collections.OrderedDict([('glac', glac_values), 
                                                                   ('dim', dim)])
        output_coords_dict['glac_acc'] = collections.OrderedDict([('glac', glac_values), 
                                                                  ('dim', dim)])
        output_coords_dict['glac_refreeze'] = collections.OrderedDict([('glac', glac_values), 
                                                                       ('dim', dim)])
        output_coords_dict['glac_melt'] = collections.OrderedDict([('glac', glac_values), 
                                                                   ('dim', dim)])
        output_coords_dict['glac_massbaltotal'] = collections.OrderedDict([('glac', glac_values), 
                                                                           ('dim', dim)])

    output_coords_dict['intercept_AAR'] =  collections.OrderedDict([('glac', glac_values), 
                                                                        ('dim', dim)])
    output_coords_dict['steady_AAR'] =  collections.OrderedDict([('glac', glac_values),
                                                                     ('dim', dim)])
    ...
    output_ds.attrs = {'source': 'PyGEMv0.2.0',
...
def create_xrdataset_binned_stats(glacier_rgi_table, dates_table, surface_h_initial, 
    ...
    output_coords_dict['bin_massbalclim_annual'] = (
            collections.OrderedDict([('glac', glac_values), ('bin',bin_values), ('year', year_values)]))
    output_coords_dict['bin_massbalclim_all'] = (
            collections.OrderedDict([('glac', glac_values), ('bin',bin_values), ('year', year_values),('sims', sims)]))
...
#                'comment': 'climatic mass balance is computed before dynamics so can theoretically exceed ice thickness'}
    ...
    output_ds_all['bin_massbalclim_all'].values[0,:,:,:] = output_glac_bin_massbalclim_annual
                ...
                if count_exceed_boundary_errors < pygem_prms.sim_iters:
                    ...
                    if pygem_prms.export_essential_data:
                        ...
                        if pygem_prms.export_extra_vars:
                            output_glac_temp_stats = np.nanmean(np.nanmean(output_glac_temp_monthly.reshape(20,12,sims), axis=1), axis=0)
                            output_glac_prec_stats = np.nanmean(np.nansum(output_glac_prec_monthly.reshape(20,12,sims), axis=1), axis=0)
                            output_glac_acc_stats = np.nanmean(np.nansum(output_glac_acc_monthly.reshape(20,12,sims), axis=1), axis=0)
                            output_glac_refreeze_stats = np.nanmean(np.nansum(output_glac_refreeze_monthly.reshape(20,12,sims), axis=1), axis=0)
                            output_glac_melt_stats = np.nanmean(np.nansum(output_glac_melt_monthly.reshape(20,12,sims), axis=1), axis=0)
                            output_glac_massbaltotal_stats = np.nanmean(np.nansum(output_glac_massbaltotal_monthly.reshape(20,12,sims), axis=1), axis=0)

                        # Output Mean
                        if pygem_prms.export_extra_vars:
                            output_ds_all_stats['glac_temp'].values[0,:] = calc_stats_AAR(output_glac_temp_stats)
                            output_ds_all_stats['glac_prec'].values[0,:] = calc_stats_AAR(output_glac_prec_stats)
                            output_ds_all_stats['glac_acc'].values[0,:] = calc_stats_AAR(output_glac_acc_stats)
                            output_ds_all_stats['glac_refreeze'].values[0,:] = calc_stats_AAR(output_glac_refreeze_stats)
                            output_ds_all_stats['glac_melt'].values[0,:] = calc_stats_AAR(output_glac_melt_stats)
                            output_ds_all_stats['glac_massbaltotal'].values[0,:] = calc_stats_AAR(output_glac_massbaltotal_stats)
                        
                        results, intercept_AAR, steady_AAR = compute_AAR(gdir, massbaltotal_monthly=output_glac_massbaltotal_monthly,
                                                                         ELA_annual=output_glac_ELA_annual,
                                                                         bin_massbalclim_annual=output_glac_bin_massbalclim_annual, 
                                                                         water_level=water_level)
                        
                        output_ds_all_stats['glac_frontalablation'] = results
                
                        output_ds_all_stats['intercept_AAR'].values[0,:]  = calc_stats_AAR(intercept_AAR);
                        output_ds_all_stats['steady_AAR'].values[0,:] = calc_stats_AAR(steady_AAR);
