...
from core.logic import run_classification

st.set_page_config(page_title="GGERC", layout="centered")

st.title("Global Glacier ELA Ratio Calculator (GGERC)")
...
if st.button("Run classification"):
    try:
        result_df, level = run_classification(
            is_debris, is_icecap, is_tidewater,
            temp, prcp, area, slope, aspect
        )

        st.success(f"Classification completed (Level {level})")
